...
export default {
    ...
    methods: {
        getData () {
            this.$http.get(process.env.JAVA_API + '/api/kline/eth_usdt', []).then(function (response) {
        ...
        getLatestData () {
            this.$http.get(process.env.JAVA_API + '/api/latest/eth_usdt', []).then(function (response) {
