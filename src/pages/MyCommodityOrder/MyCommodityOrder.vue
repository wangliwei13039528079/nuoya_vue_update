                ...
                <yd-infinitescroll :callback="loadMore" :scroll-top="false" ref="infinitescrollDemo">
                    <div class="order-content" v-for="(item, key) in tab.list" :key="key" slot="list">
                        <div class="order-content-header">
                            ...
                            <p class="header-right" v-if="item.status === 4">售后单</p>
                        ...
                        <div class="order-content-footer">
                            <div class="footer-top">
                                ...
                                <p class="footer-top-right">合计：¥{{ item.points }}</p>
                            </div>
                            <div class="footer-bottom" v-if="item.status === 1 && item.pay_status === 1">
                                ...
                                <yd-button type="hollow" shape="circle" class="right-btn" @click.native="deliver(item.id)">发货</yd-button>
                            </div>
                            <div class="footer-bottom" v-if="item.status == 4">
                                <yd-button type="hollow" shape="circle" class="left-btn" @click.native="merchantAfter(item.id)">查看</yd-button>
                                <yd-button type="hollow" shape="circle" class="right-btn" v-if="item.refund_status == 3" @click.native="confirm(item.id)">确认收货</yd-button>
                            </div>
...
export default {
    ...
    methods: {
        ...
        // 商家查看售后单详情
        merchantAfter(id){
            this.$router.push({path:'/MerchantAfter',query:{order_id:id}})
        },
        // 加载更多
        loadMore () {
            this.$api.shopsOrders({
        ...
        // 发货
        deliver(id){
            this.$router.push({path:'/DeliverGoods',query:{order_id: id}})
        },
        // 查看订单详情
        showDetail (id) {
            this.$router.push({path: '/MerchantOrderInfo', query: {order_id: id}})
        },
        // 立即付款
        pay (id) {
        ...
        confirm (orderId) {
            this.$dialog.confirm({
                mes: '确认收款后直接将货款退还至买家账户，是否继续?',
                opts: [
                    {
                        txt: '确定',
                        color: true,
                        callback: () => {
                            this.$api.merchantIsAgree({
                                order_id: orderId,
                                refund_status:1
                            }, res => {
                                if (res.status) {
    ...
    watch: {
        intTab () {
            this.$router.replace({path: '/MyCommodityOrder', query: {tab: this.intTab}})
        }
    }
}
</script>
