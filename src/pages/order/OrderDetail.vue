...
export default {
    ...
    methods: {
        ...
        pay () {
            this.$router.push({path: '/cashierdesk', query: {order_id: this.order_id, type: 1}})
        },
        // 申请售后
        afterSales () {
        ...
        logistics () {
            ...
            // console.log(this.order.delivery[0].logi_code,this.order.delivery[0].logi_no,address)
            this.$router.push({path: '/express', query: {code:this.order.delivery[0].logi_code_name,no: this.order.delivery[0].logi_no,address:address}})
        }
    }
}
</script>
