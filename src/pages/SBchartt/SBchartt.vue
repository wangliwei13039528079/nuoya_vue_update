...
function fileOnChange() {
  // let url = document.getElementById('upd_pic')
  // console.log(url)
...
function sendPic(images, imgName) {
  ...
  var images_obj = new webim.Msg.Elem.Images(
    images.File_UUID,
    imgName.split(".")[1]
  );
...
function addMsg(msg, prepend) {
  ...
  //如果是发给自己的消息
  if (!isSelfSend) {
    msgbody.style.background = "blue";
  } else {
    msghead.style.alignItems = "right";
    msgbody.style.background = "green";
  }
  //昵称  消息时间
  msghead.innerHTML =
...
export default {
  ...
  methods: {
    ...
    getSign() {
      let that = this;
      axios
        .get(" http://192.168.31.132:10010/im/usersig?uid=" + this.uid)
        .then(function(response) {
          //console.log(response.data.usersig);
    ...
    goLogin() {
      ...
      var loginInfo = {
        ...
        identifier: "001",
        userSig:
          "eJxlz11PgzAUgOF7fgXhVuNOSymLyS6YzA8CiWSOGm9IpcVVHRTWfYDxv5uhRhLP7fOenJwPy7Zt5yFeXvCiqHeVyU2npWNf2g4453*otRI5N7nbin8oj1q1Muelke2AyPM8DDBulJCVUaX6KQDQCLfiLR8ufG8TAAxT1yfjRL0MmCzSq7vrftN4Yr0Q2ftTH4V1CM9nvsFo5x7jHlXNkuIuYLcdK7pAzXu3xDf3h3TCOc32pa4O7DWbP67SMKExI*uENhHTjR9NgtlsdNKojfx9xydTQgkd6V62W1VXQ4ABeQi7cBrH*rS*AKeLW5Q_",
      ...
      webim.login(
        ...
        function(resp) {
          ...
          // that.chartId = localStorage.getItem("chartId");
          // setInterval(function() {
          //   that.newMsgList = that.$store.state.limsg;
          // }, 500);
    ...
    onSendMsg() {
      ...
      console.log(msg);
      // if (that.$store.state.msgList.baseMsg.mmsg) {
      //   that.$store.state.msgList.baseMsg.mmsg.push(msg);
      // } else {
      //   that.$store.state.msgList.baseMsg.mmsg = [];
      //   that.$store.state.msgList.baseMsg.mmsg.push(msg);
      // }
...
console.log("1045")
          ...
          that.getHistoryMsgCallback(resp.MsgList);
        },
        cbError
      );
    },
      ...
      webim.syncGroupMsgs(
        ...
        function(msgList) {
          ...
          if (cbOk) {
            ...
            that.getHistoryMsgCallback(msgList, true);
          }
        },
        function(err) {
          // alert(err.ErrorInfo);
    ...
    getLastGroupHistoryMsgs(cbOk) {
      ...
      getGroupInfo(selToID, function(resp) {
        ...
        recentSessMap[
          webim.SESSION_TYPE.GROUP + "_" + selToID
        ].MsgGroupReadedSeq =
          resp.GroupInfo && resp.GroupInfo[0] && resp.GroupInfo[0].MsgSeq;
        webim.syncGroupMsgs(
          options,
          function(msgList) {
            if (msgList.length == 0) {
    ...
    // onMsgNotify(newMsgList) {
    //   this.$store.state.limsg = newMsgList;
    //   console.log(this.$store.state.limsg);
    // },
    fileOnChange() {
