<template>
  <div id="SBchart">
    ...
    <div class="bigImg" @click="goHide()" ref="fff"></div>
    <div class="msgflow" ref="scroll"></div>
    <div
      v-show="showProp==1"
      style="position: fixed;left: 50%;top: 50%;width:260px;height:260px;background:#fff;margin-left:-130px;margin-top:-130px;color:#000;padding:5px;"
    >
      <div class="modal-body">
        ...
        >
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <input id="upd_pic" @change="fileOnChange()" type="file">
      ...
      <div class="modal-footer">
        <button class="btn btn-default" data-dismiss="modal" type="button" @click="goClose()">关闭</button>
        <button class="btn btn-primary" @click="uploadPic()" type="button">发送</button>
      </div>
    </div>
    <!-- </div> -->
...
Vue.use(Toast);
import "../../../static/js/image";
// import common from '../../common/common'
import {im} from '../../common/apigo'
//获取跟谁聊天的id
let loginInfo = JSON.parse(common.getStorage("loginSign"));
...
function selectPicClick() {
  ...
  if (
    ...
    parseInt(webim.BROWSER_INFO.ver) <= 9
  ) {
    //if(1==1){
    $("#updli_form")[0].reset();
...
function sendPic(images) {
  // let selSess;
  let selToID = loginInfo.identifier;
  // console.log(selToID)
  if (!selToID) {
...
function imageClick(imgObj) {
  ...
  var dddy = document.getElementsByClassName("msgflow")[0];
  dddy.appendChild(dddy);
  bigPicture.innerHTML = "";
  var span = document.createElement("span");
  span.innerHTML = '<img class="imgBig" src="' + bigImgUrl + '" />';
}
...
function addMsg(msg, prepend) {
  ...
  sessType = msg.getSession().type(); //获取消息所属的会话
  isSelfSend = msg.getIsSend(); //消息是否为自己发的
  fromAccount = msg.getFromAccount(); //获取消息者昵称
  ...
  if (isSelfSend) {
    ...
    // }
    //获取头像
    // console.log(loginInfo.headurl)
    // if (loginInfo.headurl) {
      fromAccountImage = JSON.parse(common.getStorage("loginSign")).headurl;
    // } else {
    ...
    // } else {
      fromAccountImage=common.getStorage("Photo")
    // }
  }
  ...
  //如果是发给自己的消息
  if (!isSelfSend) {
     msgbody.style.background = "white";

    msgbody.style.color = "#000";
  } else {
    // console.log(msgPre.children)
    onemsg.style.display = "flex";
      msgbody.style.background = "#9eea6a";

    onemsg.style.flexFlow = "row-reverse";
    msghead.style.float = "right";

    msgbody.style.color = "#000";
  }
  //昵称  消息时间
  msghead.innerHTML =
    "<img class='headurlClass' src='" + fromAccountImage + "'>";
...
function convertSoundMsgToAMRPlayer(content) {
  ...
  var aElmentString = [
    '<button id="',
    btnid,
    '" style="font-size:1.5em;" data-url="',
    content.downUrl,
    '">',
    iconStartChar,
    "</button>"
...
function convertGroupTipMsgToHtml(content) {
  ...
  switch (opType) {
    ...
    case webim.GROUP_TIP_TYPE.QUIT: //退出群
      text +=
        opUserId + "离开该群，当前群成员数：" + content.getGroupMemberNum();
      break;
...
export default {
  ...
  methods: {
    //查看图片
    goHide() {
      this.$refs.fff.style.display = "none";
    },
    goClose() {
      this.showProp = 0;
    },
    ...
    getSign() {
      ...
      axios
        ...
        .then(function(response) {
          that.usersig = response.data.usersig;
          that.goLogin();
        });
    },
    //登录
    goLogin() {
      let that = this;
      var loginInfo = {
        sdkAppID: 1400208374, //用户所属应用id,必填
        accountType: 36862, //用户所属应用帐号类型，必填 , 已废弃
        identifier: that.uid, //当前用户ID,必须是否字符串类型，必填
        userSig: this.usersig,
        identifierNick: null, //当前用户昵称，不用填写，登录接口会返回用户的昵称，如果没有设置，则返回用户的id
        headurl: this.avatarUrl //当前用户默认头像，选填，如果设置过头像，则可以通过拉取个人资料接口来得到头像信息
      };
      this.GLOBAL.setStorage("loginSign", loginInfo);
      // console.log(loginInfo);
      var listeners = {
        onConnNotify: that.onConnNotify, //监听连接状态回调变化事件,必填
        jsonpCallback: that.jsonpCallback, //IE9(含)以下浏览器用到的 jsonp 回调函数，
    ...
    onSendMsg() {
      ...
      webim.sendMsg(
        ...
        function(resp) {
          ...
          webim.Tool.setCookie("tmpmsg_" + that.selToID, "", 0);
          that.sendMsg = "";
        },
        function(err) {
          // alert(err.ErrorInfo);
    ...
    //接收消息,监听新消息事件,newMsgList 为新消息数组，结构为[Msg]
    onMsgNotify(newMsgList) {
      ...
      for (var j in newMsgList) {
        //遍历新消息
        newMsg = newMsgList[j];
        if (newMsg.getSession().id() == that.selToID) {
          //为当前聊天对象的消息
    ...
    getLastC2CHistoryMsgs(cbOk, cbError) {
      ...
      var msgKey = "";
      var options = {
        Peer_Account: that.selToID + "", //好友帐号，selToID 为全局变量，表示当前正在进行的聊天 ID，当聊天类型为私聊时，该值为好友帐号，否则为群号。
        MaxCnt: 15, //拉取消息条数
    ...
    getHistoryMsgCallback(msgList, prepage) {
      ...
      for (var j in msgList) {
        //遍历新消息
        msg = msgList[j];
        if (msg.getSession().id() == that.selToID) {
          //为当前聊天对象的消息
    ...
    fileOnChange() {
      let uploadFile = document.getElementById("upd_pic");
      // console.log(url)
    ...
    uploadPic() {
      ...
      //封装上传图片请求
      // let loginInfo = common.getStorage("loginSign");
      // console.log(loginInfo)
      var opt = {
        file: file, //图片对象
    ...
    captureImage() {
      ...
      cmr.captureImage(
        function(capturedFile) {
          alert("Capture image success: " + capturedFile);
          plus.io.resolveLocalFileSystemURL(
            capturedFile,
            function(a) {
              a.file(function(file) {
                file = file;
                alert("Capture image success: " + file);
              });
              // var file = a.toLocalURL();
              // alert("Capture image success: " + file);
              var businessType; //业务类型，1-发群图片，2-向好友发图片
              if (selType == webim.SESSION_TYPE.C2C) {
                //向好友发图片
