    ...
    <div class="goodslist">
        ...
        <yd-infinitescroll :callback="loadMore" :distance="2" :scroll-top="false" ref="infinitescrollDemo" class="itemgoodslist">
            <yd-list theme="4" slot="list">
                <yd-list-item v-for='(item, key) in list' :key='key' type="link" :href="{path:'goodsdetail',query:{goods_id:item.id}}" v-if="Number(item.price) > 0">
                    <img slot='img' :src='item.image_url'>
                    <span slot='title'>{{item.name}}</span>
                    <yd-list-other slot='other'>
                        <div>
                            <span class='demo-list-price' style="color:#ababab"><em></em>{{item.price}}</span>
                        </div>
                    </yd-list-other>
                </yd-list-item>
            </yd-list>
...
<style>
    ...
    .goodslist .yd-list-theme4 {
        background-color: #000 !important;
    }
    .goodslist .yd-list-mes {
        background-color: #000 !important;
    }
    .goodslist .yd-list-title {
        color: #fff;
    }
    .goodslist .yd-list-item {
        border: none;
    }
    .goodslist .yd-list-donetip {
