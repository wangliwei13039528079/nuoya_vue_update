<template>
    <div id="paypassword">
        <HeaderTop title="支付密码设置">
            <a class="back" @click="$router.back(-1)" slot="left">
                <i class="iconfont iconfanhui"></i>
            </a>
        </HeaderTop>
        <van-cell-group>
            <van-field  readonly placeholder="请输入旧的支付密码" v-model="oldPassword" type="password" maxlength="6"
                       @click="flag='old';show=true" right-icon="closed-eye" @click-right-icon="oldShow=false" v-show="oldShow"/>
            <van-field  readonly placeholder="请输入旧的支付密码" v-model="oldPassword" type="text" maxlength="6"
                       @click="flag='old';show=true" right-icon="eye-o" @click-right-icon="oldShow = true" v-show="!oldShow"/>

            <van-field  readonly type="password" placeholder="请输入新密码" v-model="newPassword" maxlength="6"
                       @click="flag='new';show=true" right-icon="closed-eye" @click-right-icon="newShow=false" v-show="newShow"/>
            <van-field  readonly type="text" placeholder="请输入新密码" v-model="newPassword" maxlength="6"
                       @click="flag='new';show=true" right-icon="eye-o" @click-right-icon="newShow = true" v-show="!newShow"/>
            ...
            async confirmEdit() {
                if (this.newPassword == this.confirmPassword) {
                    ...
                    if(oldPassword == newPassword){
                        return Toast('新密码和旧密码不能一致')
                    }
                    this.$api.editPayPwd({newPassword,oldPassword,key},res=>{
                        if(res){
